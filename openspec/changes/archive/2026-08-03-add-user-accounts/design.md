...
- [Abuse, not usage, is the real cost risk — this is the first surface where a bug can cost money] → Mitigated by API Gateway stage throttling (20 rps / 40 burst, 5/10 on the public share route), a 200-card per-user cap, request body size validation, and 14-day log retention (CloudWatch Logs can quietly become the largest line item). An AWS Budgets alert is a manual prerequisite. **Reserved concurrency is not currently set**: AWS rejects any reservation that would leave the account's unreserved pool below 100, and this account has only 50 available (a reduced initial limit AWS raises over time, despite the nominal quota reading 1000). The `lambda_reserved_concurrency` variable exists to restore the ceiling once the limit allows, defaulting to `-1`. Accepted knowingly — the stage throttle is the binding control, and concurrency was defense in depth.
