...
- **Unchanged:** the card renderer, the stored card shape, and both contract tests; every existing route and its authorization; the signed-out experience of every card capability, none of which is affected by a footer; the brand seam's rules, which the new copy simply obeys; and every Terraform *resource* — the only infrastructure edit is one entry in the API's route map, which creates no new resource type and touches no existing route.
...
- **Infra** (`infra/`): one line — the route must be added to `local.api_routes` in `apigateway.tf`. API Gateway's routes are enumerated in Terraform, so a route absent from that map 404s before the Lambda is ever invoked, however complete the backend code is. No other infrastructure changes: the table's TTL is already enabled on `expiresAt`, and an authenticated route needs no stricter throttle than the stage default. **This means a Terraform apply per workspace is a prerequisite to the feature working**, alongside the code deploy.
