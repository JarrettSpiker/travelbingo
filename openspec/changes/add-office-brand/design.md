...
- **The Google OAuth consent screen carries the wrong brand, and that is accepted.** A Google Cloud project has exactly one consent screen, so two brands sharing a project means an Office Lingo Bingo visitor is asked to grant access to "Travel Bingo", with that app name, logo, and privacy links. The original plan was a second Google Cloud project to avoid this. **Decided otherwise:** the repo owner judged the mismatch acceptable for a satire site, against the cost of a second project and a second consent screen to maintain. → Worth being precise about what is and is not affected. **Account separation is untouched**: identity is the Cognito `sub`, each brand has its own user pool, and every item is keyed `USER#<sub>` in that stack's own table — so the same person signing into both sites gets two unrelated accounts, and a travel token still cannot authenticate against the office API. The Google project plays no part in that isolation. What is affected is only what the user *reads* on the consent screen. Each environment still gets its **own OAuth client** within the shared project, so one stack's credentials can be rotated or revoked without touching another's.
