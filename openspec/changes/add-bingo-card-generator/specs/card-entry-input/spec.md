...
The system SHALL let the user mark or unmark an individual entry as mandatory. Mandatory entries are guaranteed to appear on the card whenever the pool exceeds the grid's 24-entry capacity (see the card-generation capability for selection behavior).
...
The system SHALL prevent the same text entry (case-insensitive, trimmed) from being added to the pool more than once.
...
The system SHALL let the user specify the text shown in the center "free" space of the grid, separate from the entry pool.
